...
namespace IBLeier.VictronEnergy.ModbusTcp
{
	...
	internal class ModbusClient
	{
		...
		public bool Connected
		{
			get
			{
				return (this.masterTcpClient != null) && this.masterTcpClient.Connected;
			}
		}
		...
		{
			ushort[] result = this.master.ReadHoldingRegisters(this.UnitIdentifier, (ushort)startingAddress, (ushort)quantity);
		...
		public void WriteMultipleRegisters(int startingAddress, int[] values)
		{
			...
			this.master.WriteMultipleRegisters(this.UnitIdentifier, (ushort)startingAddress, data);
		}
	}
}